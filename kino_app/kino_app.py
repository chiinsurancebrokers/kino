...
def live_refresh():
    ...
    and resolve any pending tracked picks against the fresh data. Returns
    a dict summary for display - never raises, always returns an 'error'
    key on failure so the UI can show it plainly.
    """
    current_df = get_data()
    if current_df.empty:
    ...
    if error:
        return {"error": error}
    if new_draws.empty:
        return {"new_draws": 0, "resolved_picks": 0}
    ...
    return {"new_draws": len(new_draws), "resolved_picks": n_resolved}
...
def page_pick_generator(analyzer):
    ...
    if st.button("💾 Track this pick against future draws"):
        ...
        kino_tracking.save_pending_pick(pick, max_draw_id, source="engine")
        st.success(
...
def page_ai_insights(analyzer):
    ...
    if st.button("💾 Track this pick against future draws", key="_kino_track_ai_pick"):
        max_draw_id = int(analyzer.all_draws["draw_id"].max())
        kino_tracking.save_pending_pick(result["numbers"], max_draw_id, source="claude")
        st.success(f"Saved. Check the Track Record tab after fetching new draws.")
...
def page_track_record(analyzer):
    ...
    if last_result:
        if last_result.get("error"):
            ...
            st.success(f"Fetched {last_result.get('new_draws', 0)} new draw(s), "
