...
**Continuous auto-tracking (new):** every time you fetch new draws, the app
now closes the loop itself — resolves whatever pending pick was waiting,
and if nothing's queued, immediately generates and saves the next KINO 7
pick for the following draw. There is always exactly one auto-generated
pick in flight; refreshing repeatedly doesn't pile up duplicates (verified
across 3 consecutive refreshes in testing — exactly one pending pick at
every step, resolved count growing by one each time). This is what your
first real result (matched somewhere between 3 and 6 of 7) will keep
happening against automatically from here, without needing a manual
"track this pick" click after every draw.

## Real production findings (from your first live run)

Confirmed the API endpoint IS live and responding post-Allwyn-rebrand — a
huge deal, since this was unverifiable at build time. But a single request
for a 2000-draw range came back `400 Bad Request`. Fixed: `fetch_new_draws`
now fetches in chunks, starting at 100 and adaptively halving (down to a
floor of 10) whenever a chunk specifically 400s, retrying that same
sub-range at the smaller size — so it discovers the real limit instead of
guessing once and breaking on any large backlog. Tested this against a
mock simulating a hard 10-draw cap: correctly recovered all 55 requested
draws despite starting 10x too optimistic, with correct chronological
ordering preserved across every chunk boundary.

Also fixed: clicking "Track this pick" twice before the next draw arrives
was creating duplicate pending entries (visible in your screenshot — two
identical rows). `save_pending_pick` now skips an exact duplicate (same
numbers, source, and target draw) rather than saving it again — verified
this still allows genuinely different picks (different numbers, or the
same numbers from a different source like Claude vs. the engine) to
coexist as separate pending entries, just not literal re-clicks of the
same thing.
