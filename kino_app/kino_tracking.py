...
def save_pending_pick(numbers, generated_at_draw_id, source="engine", filepath=TRACKING_FILE_DEFAULT):
    """source: 'engine' (deterministic scoring), 'engine_auto' (auto-queued by
    live_refresh), or 'claude' (AI Insights pick). Skips saving an exact duplicate
    (same numbers, source, and generated_at_draw_id already pending) - repeated
    clicks before the next draw arrives shouldn't pile up redundant entries all
    targeting the same future draw. Returns True if saved, False if skipped as a
    duplicate."""
    data = _load(filepath)
    for p in data["pending"]:
        if (p["source"] == source and p["generated_at_draw_id"] == generated_at_draw_id
                and sorted(p["numbers"]) == sorted(numbers)):
            return False
    data["pending"].append({
